...
stage** (`text.proofread`); until then every work is `proofread:false` (the pre-public
gate). Only `public-domain` / `permission-granted` works are published (`rights.status`);
the archive's rights position is stated in `Rights.md` + `LICENSE`.
...
- **metadata.json**: conform to `metadata.schema.json` (no `rights.verified` field — the
  ...
  1959; Nepal's Copyright Act 2059 grants life + 50 years, expiring 2009.").
