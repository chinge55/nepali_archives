...
pipeline/
  ...
  build_index.py            # regenerate index.json from every metadata.json
  build_site.py             # build the static reader site/ from index.json + works (CI runs this)
  subset_fonts.py           # subset the Noto Serif Devanagari woff2 to glyphs the site uses
...
author directory equals `author.id`. Slugs are best-effort **natural-Nepali
romanization** via `pipeline/devanagari_slug.py` (drop word-final inherent schwa
unless after a conjunct; व→b, श/ष→sh) — review them, they're a starting point.
`index.json` is regenerated by `pipeline/build_index.py` (nothing else writes it).
